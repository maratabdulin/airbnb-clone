...
export class PageObject<T> {
  ...
  protected getByAutomationIdNative(automationId: string): Node | null {
    return (
      this.fixture.nativeElement.querySelector(
  ...
  protected getAllByAutomationIdNative(automationId: string): Node[] {
    const node: NodeList = this.fixture.nativeElement.querySelectorAll(
      `[automation-id="${automationId}"]`,
    );
    const arr: Node[] = [];
    node.forEach((item) => {
  ...
  protected text(element: DebugElement | string): string | null {
    const el =
      element instanceof DebugElement
        ? element
        : this.getByAutomationId(element);
    if (!el) {
      return null;
    }
  ...
  protected triggerEventHandler(
    ...
    if (element !== null) {
      ...
      console.warn("Element of triggerEventHandler is NULL");
    }
  }
}
