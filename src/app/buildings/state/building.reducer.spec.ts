...
describe("BuildingReducer", () => {
  const getState = (payload?: Partial<BuildingState>): BuildingState =>
    createState(buildingInitialState, payload);

  it("loadBuildings() should clear buildingLoadError", () => {
  ...
  it("loadBuildingsSuccess() should set buildings", () => {
    ...
    const action = BuildingActions.loadBuildingsSuccess({
  ...
  it("loadingBuildingsFailure() should set buildingsLoadError", () => {
    const state = getState();
    const action = BuildingActions.loadBuildingsFailure({
  ...
  it("clearBuilding() should clears buildings", () => {
  ...
  it("removeBuilding() should clear buildingLoadError", () => {
    const state = getState({ ...createEntityState(BUILDINGS_STUB) });
  ...
  it("addBuildingSuccess() should add buildings", () => {
    ...
    const action = BuildingActions.addBuildingSuccess({
      payload: BUILDING_STUB,
    });
    const result = reducer(state, action);
  ...
  it("addBuildingFailure() should set buildingCreateError", () => {
    const state = getState();
    const action = BuildingActions.addBuildingFailure({
      payload: BUILDINGS_LOAD_ERROR,
    });
    const result = reducer(state, action);
  ...
  it("changeBuilding() should clear buildingLoadError", () => {
    const state = getState({ ...createEntityState(BUILDINGS_STUB) });
  ...
  it("changeBuildingSuccess() should set buildingChangeRun to false", () => {
    const state = getState({ ...createEntityState(BUILDINGS_STUB) });
    const action = BuildingActions.changeBuildingSuccess({
      payload: ENTITY_STUB,
    });
    const result = reducer(state, action);
    const entity = result.entities[ENTITY_STUB.id] as BuildingEntity;
  ...
  it("changeBuildingFailure() should set buildingChangeError", () => {
    const state = getState({ ...createEntityState(BUILDINGS_STUB) });
    const payload = { ...BUILDINGS_LOAD_ERROR, ...ENTITY_STUB };
