...
describe("Booking Reducer", () => {
  const getState = (payload?: Partial<BookingState>): BookingState =>
    createState(bookingInitialState, payload);

  it("setBookingVariant() should set bookingVariant", () => {
    const state = getState();
    const action = BookingActions.setBookingVariant({
      payload: BOOKING_VARIANT_STUB,
    });
  ...
  it("setBookingDetails() should set bookingDetails", () => {
    const state = getState();
    const action = BookingActions.setBookingDetails({
      payload: BOOKING_DETAILS_STUB,
    });
    const result = reducer(state, action);
