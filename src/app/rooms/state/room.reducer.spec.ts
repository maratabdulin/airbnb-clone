...
describe("RoomReducer", () => {
  const getState = (payload?: Partial<RoomState>): RoomState =>
    createState(roomInitialState, payload);
  ...
  it("clearRooms() should clear rooms", () => {
  ...
  it("addRoom() should set roomCreateRun true", () => {
    ...
    const result = reducer(state, action);
    expect(result.roomsCreateRun).toBeTruthy();
  });
  ...
  it("addRoomFailure() should set roomCreateError", () => {
    ...
    const result = reducer(state, action);
    expect(result.roomsCreateError).toEqual(ROOM_LOAD_ERROR);
  });

  it("changeRoom() should clear roomLoadError", () => {
    const state = getState({ ...createEntityState(ROOMS_STUB) });
  ...
  it("changeRoomFailure() should set roomChangeError", () => {
    const state = getState({ ...createEntityState(ROOMS_STUB) });
    const payload = { ...ROOM_LOAD_ERROR, ...ENTITY_STUB };
