...
export class PersonEffects implements OnInitEffects {
    ...
    this.actions$.pipe(
      ...
      fetch({
        ...
        onError: (action, payload) =>
          PersonActions.loadPersonsFailure({ payload }),
      }),
    ),
  );
      ...
      fetch({
        ...
        run: (action, persons) =>
          PersonActions.addPersonSuccess({
            payload: createPersonFromNewPerson(persons ?? [], action.payload),
          }),
        onError: (action, payload) =>
          PersonActions.addPersonFailure({ payload }),
      }),
    ),
  );
  ...
  removePerson$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        this.store.pipe(select(PersonSelectors.selectPersonsEntities)),
      ),
      fetch({
        id: (action) => `remove-person-${action.payload.id}`,
        run: (action, personEntities) => {
          const person = personEntities
            ? personEntities[action.payload.id]
            : null;
          return person
            ? PersonActions.removePersonSuccess({ payload: action.payload })
            : PersonActions.removePersonCancel();
        },
  ...
  changePerson$ = createEffect(() =>
    this.actions$.pipe(
      ...
      fetch({
        ...
        run: (action) =>
          PersonActions.changePersonSuccess({ payload: action.payload }),
        onError: (action, payload) =>
          PersonActions.changePersonFailure({ payload }),
      }),
    ),
  );
  ...
  removePersonBuilding$ = createEffect(() =>
    this.actions$.pipe(
      ...
      fetch({
        id: (action) =>
          `remove-person-building-${action.payload.id}-${action.payload.building}`,
        run: (action, personEntities) => {
          const person = personEntities
            ? personEntities[action.payload.id]
            : null;
          ...
          return PersonActions.removePersonBuildingSuccess({
  ...
  addPersonBuilding$ = createEffect(() =>
    this.actions$.pipe(
      ...
      fetch({
        id: (action) =>
          `add-person-building-${action.payload.id}-${action.payload.building}`,
        run: (action, personEntities) => {
          const person = personEntities
            ? personEntities[action.payload.id]
            : null;
  ...
  clearPersonsBuildings$ = createEffect(() =>
    this.actions$.pipe(
      ...
      fetch({
        ...
        onError: (action, payload) =>
          PersonActions.clearPersonsBuildingsFailure({ payload }),
      }),
    ),
  );
  ...
  localStorageSync$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(this.store.pipe(select(PersonSelectors.selectPersons))),
      fetch({
        id: (action) => `local-storage-sync-${action.type}`,
        run: (action, persons) => {
