...
describe("PersonReducer", () => {
  const getState = (payload?: Partial<PersonState>): PersonState =>
    createState(personInitialState, payload);
  ...
  it("loadPersonsFailure() should set personsLoadError", () => {
    const state = getState();
    const action = PersonActions.loadPersonsFailure({
      payload: PERSON_LOAD_ERROR,
    });
    const result = reducer(state, action);
    expect(result.personsLoadError).toEqual(PERSON_LOAD_ERROR);
  });

  it("clearPersons() should clear persons", () => {
  ...
  it("addPersonFailure() should set personCreateError", () => {
    const state = getState();
    const action = PersonActions.addPersonFailure({
      payload: PERSON_LOAD_ERROR,
    });
    const result = reducer(state, action);
